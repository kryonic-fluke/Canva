...
const getNodeText = (node: Node): string => {
  ...
  if (node.type === 'checklist') {
    ...
    const itemsText = (node.data.items || []).map((item: ChecklistItem ) => item.text).join(', ');
...
export async function categorizeNodeAPI(nodesToCategorize: Node[]) {
 ...
 const categories = ["Bug Report", "Feature Request", "Question", "Task", "Key Insight", "Decision",'anticipation',"immediate","moat","Prescient"];
  const validNodes = nodesToCategorize.filter(node => getNodeText(node).trim() !== '');
