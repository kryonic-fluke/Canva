...
import { useRemoveContributor } from "../hooks/useRemoveContributor";
import { ArrowRightOnRectangleIcon, ShareIcon, TrashIcon } from '@heroicons/react/24/outline'; 
...
export const CanvasListItem = ({
  ...
  const { mutate: leaveCanvas, isPending: isLeaving } =
...
const handleOpenConfirmModal = () => {
  ...
  const handleConfirmLeave = () => {
    if (!canvasId || !userId) {
 console.error("Cannot leave canvas: canvasId or userId is missing.");
      return;
    }
      const paramsToSend = {
      canvasId: canvasId,
  ...
  return (
        <>
    <li className="my-2 p-3 bg-gray-800 rounded-lg shadow-lg hover:bg-gray-700/50 transition-colors duration-200">
 ...
 <ShareIcon className="h-6 w-6" />
              ...
              <button onClick={handleOpenConfirmModal} disabled={isLeaving}>
                {!isLeaving ?              <ArrowRightOnRectangleIcon className="h-6 w-6 hover:text-red-500" />
...
: "Leaving..."}
    ...
    <ConfirmModal
        isOpen={isModalOpen}
        title="Confirm Leave"
        message="Are you sure you want to leave this canvas? This action cannot be undone."
        onConfirm={handleConfirmLeave}
        onCancel={handleCancel}
        confirmText={isLeaving ? "Leaving..." : "Yes, Leave"}
      />
      </>
